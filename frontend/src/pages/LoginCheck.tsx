...
export default function LoginCheck() {
  //http://localhost:3000/logined?accessToken=jiyoung&refreshToken=1111&memberId=3
  ...
  useEffect(() => {
    ...
    const memberNickname = url.searchParams.get("nickname")
    if (accessToken && refreshToken && memberId && memberNickname) {
      localStorage.setItem("accessToken", accessToken)
      localStorage.setItem("refreshToken", refreshToken)
      localStorage.setItem("memberId", memberId)
      localStorage.setItem("memberNickname", memberNickname)

      window.location.replace(`/room/${memberId}`)
    }
  }, [])
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden">
      ...
      <div className="absolute flex flex-col items-center justify-center transform -translate-x-1/2 -translate-y-1/2 gowunBatang top-1/2 left-1/2">
        <div className="mb-5 text-white">로그인 중입니다.</div>
        <div className=" w-[300px] h-[50px] mb-5 flex rounded-lg cursor-pointer justify-center">
          <svg
            aria-hidden="true"
